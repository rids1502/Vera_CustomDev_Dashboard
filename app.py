...
# Column indices (0-based) — update here if sheet columns change
COL_FEED_ID       = 0
COL_PARENT_ID     = 1
COL_PARENT_NAME   = 2
COL_STATUS        = 3
COL_CREATED_DATE  = 4   # Parent.CreatedDate  ← NEW
COL_INSERTED_BY   = 5
COL_BODY          = 6
COL_CMT_ID        = 7
COL_CMT_BODY      = 8
...
.chat-ai {
    ...
    font-size: 13px; line-height: 1.5; border: 1px solid rgba(0,0,0,0.08);
}
...
def parse_date(date_str):
    """Parse ISO date string → (YYYY-MM-DD, datetime object, 'Month YYYY')"""
...
# ── Persistence ───────────────────────────────────────────────────────────────
STORAGE_KEY = "vera_action_states_v2"

def load_persistent_states():
    if not st.session_state.get("persistent_loaded"):
        st.session_state.persistent_loaded = True
        raw = st.session_state.get(STORAGE_KEY, "{}")
        try:
            st.session_state.action_states = json.loads(raw) if isinstance(raw, str) else raw
        except:
            st.session_state.action_states = {}

def save_persistent_states():
    st.session_state[STORAGE_KEY] = json.dumps(st.session_state.action_states)
...
def load_deliverables():
    ...
    # Sort by deliverable creation date — newest first
...
def call_gemini(prompt, system=""):
    ...
    try:
        ...
        raise Exception(f"Gemini returned a non-JSON response (HTTP {resp.status_code}). Check your API key.")
    if not resp.ok:
...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("ai_cache",{}),("chat_history",[]),
             ("selected_idx",None),("persistent_loaded",False)]:
    if k not in st.session_state: st.session_state[k] = v
if "action_states" not in st.session_state:
    st.session_state.action_states = {}

load_persistent_states()
    ...
    if idx is None:
        ...
        if not is_rejected and idx not in st.session_state.ai_cache:
            with st.spinner("Analysing thread with Gemini..."):
                try:
                    ...
                    if idx_key not in st.session_state.action_states:
        ...
        with st.container(border=True):
            ...
            if is_rejected:
                ...
                st.caption("Select a deliverable to generate summary.")
        ...
        with st.container(border=True):
            ...
            elif ai or idx_key in st.session_state.action_states:
                acts    = st.session_state.action_states.get(idx_key, [])
                changed = False
                    ...
                    with cb1:
                        if action.get("editing"):
                            if st.button("Save", key=f"save_{idx}_{i}"):
                                st.session_state.action_states[idx_key][i]["text"]    = new_val
                                st.session_state.action_states[idx_key][i]["editing"] = False
                                changed = True
                        elif action.get("confirmed"):
                            if st.button("Undo", key=f"undo_{idx}_{i}"):
                                st.session_state.action_states[idx_key][i]["confirmed"] = False
                                changed = True
                        else:
                            if st.button("✓ Confirm", key=f"conf_{idx}_{i}"):
                                st.session_state.action_states[idx_key][i]["confirmed"] = True
                                changed = True
                    ...
                    with cb2:
                        if action.get("editing"):
                            ...
                            if st.button("Edit", key=f"edit_{idx}_{i}"):
                                st.session_state.action_states[idx_key][i]["editing"] = True
                                changed = True

                    with cb3:
                        if not action.get("editing"):
                            if st.button("🗑", key=f"del_{idx}_{i}", help="Delete action item"):
                                st.session_state.action_states[idx_key].pop(i)
                                changed = True
...
with col_chat:
    ...
    st.caption("Ask about any deliverable or the full pipeline")
    st.markdown("---")

    all_ctx = "\n\n---\n\n".join(
        f'"{d["name"]}" | {d["status"]} | Created: {d["created_date"]} | {d["month_label"]}\n{d["thread"]}'
    ...
    with chat_container:
        if not st.session_state.chat_history:
            st.markdown(
                "<div class='chat-ai'>Hi! I have context on all deliverables grouped by creation month. "
                "Ask me anything — what's blocked, who needs follow-up, "
                "or ask for a leadership summary.</div>",
                unsafe_allow_html=True)
        for msg in st.session_state.chat_history:
            if msg["role"] == "user":
                ...
                st.markdown(
                    f"<div class='chat-label'>Gemini</div>"
